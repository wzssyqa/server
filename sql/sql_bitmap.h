...
/* Use compiler intrinsics , or "builtins" for bit scanning if available.*/
#if defined(_MSC_VER)
#include <intrin.h>
#if defined(_M_IX86)
...
#ifdef HAVE_BITSCAN_INTRINSICS
  /*
    Find the position of the first(least significant) bit set in the
    bmp argument.
  */
  static uint bit_scan(ulonglong num)
  {
#if defined(__GNUC__)
    ...
    unsigned long bit;
#if defined(_M_IX86)
    if (_BitScanForward(&bit, (uint) num))
      return bit;
    _BitScanForward(&bit, (uint)(num >> 32));
    return bit + 32;
#else
    _BitScanForward64(&bit, num);
    return bit;
#endif
#endif
  }
