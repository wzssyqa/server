  ...
  */
  ...
  if (!lex->stmt_var_list.is_empty() && !thd->slave_thread)
  {
    ...
    */
    ...
    /*
      SET STATEMENT syntax is applicable only on master server. The SET
      STATEMENT part is not effective on slave server. Hence replicate only
      the rest of the query which is effective on slave. Please refer
      MDEV-14760.

      Look for the first "FOR" token in SET STATEMENT.

      sample_query:
      SET STATEMENT auto_increment_increment=10 FOR INSERT INTO t1 VALUES(@@auto_increment_increment);

      Extract the rest of query string after the "FOR" keyword and set is as
      new thd->query() so that during replication only the actual query gets
      replicated.

      For example the extracted query will be: INSERT INTO t1 VALUES(@@auto_increment_increment)
    */
    char pattern[]= "FOR";
    char *pattern_start_pos= NULL;
    char *sub_query_start_pos= NULL;
    char *new_query=NULL;
    String sub_query;
    char *query= thd->query();
    pattern_start_pos= strstr(query, pattern);
    do
    {
      if (pattern_start_pos)
      {
        /*
           Ignore "FOR" token, if it is part of SET STATEMENT var_list.

           For example:
           SET STATEMENT sql_mode='ERROR_FOR_DIVISION_BY_ZERO' FOR INSERT INTO t2 VALUES(@@sql_mode);

           In above query sql_mode var has "FOR" as substring. Ignore such
           cases.
        */
        if ((*(pattern_start_pos-1) == ' ' || *(pattern_start_pos-1) == '\n') &&
            (*(pattern_start_pos+3) == ' ' || *(pattern_start_pos+3) == '\n'))
        {
          sub_query_start_pos= pattern_start_pos + 3;
          break;
        }
        else
          query= pattern_start_pos + 3;
      }
    } while((pattern_start_pos=strstr(query, pattern)));

    if (sub_query_start_pos)
    {
      // Trim left ' ' and '\n'.
      while (*sub_query_start_pos == ' ' || *sub_query_start_pos == '\n')
        sub_query_start_pos++;
      sub_query.append(sub_query_start_pos,strlen(sub_query_start_pos));
      sub_query.append(" /* generated by server */");
      new_query= (char *)thd->strmake(sub_query.ptr(), sub_query.length());
      thd->set_query(new_query, strlen(new_query));
    }
    else
    {
      my_error(ER_SET_STATEMENT_QUERY_REWRITE_FAILED, MYF(0), thd->query());
      goto error;
    }
  }
